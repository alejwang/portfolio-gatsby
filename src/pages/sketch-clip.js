...
import NextNav from '../components/next'
import { ChapterTitle, SectionForImages, SectionForImagesFlex, TitleGroup, BannerGroup, Title, Subtitle, InfoPanelGroup, InfoGroup, InfoTitle, InfoText, Image, Section, SectionTitle, MainText, StepContainerGroup, StepTitle, StepNo, StepText, SmallTitle, InlineImage } from '../components/template'
import DeviceMockUp from '../components/devicemockup'

const workId = 'Sketch Clip'
const workTitle = 'Sketch Clip'

const SketchClip = (props) => (
  <div>
    <Helmet
    ...
    <Section>
      ...
      <SectionTitle>Sketch Clip is one place for version control + seamless collaboration experience, designed for Sketch app.</SectionTitle>
    ...
    <SectionForImagesFlex>
      <DeviceMockUp macbook video={require("../images/sketch-clip-prototype.mp4")} text="0:07 - Create a new project by analyzing the local files.  0:25 - Show versions and branchers and design changes. 0:57 - Pull new versions, deal with conflicts and push new changes to the cloud."/>
    </SectionForImagesFlex>
    ...
    <Section>
      ...
      <StepContainerGroup>
        ...
        <StepText>Two of 5 interviewees (as UX Designers) mentioned they had trouble finding a past version or resources to reuse. The main reason is they can't remember what they did days before, and some versions only have tiny differences.</StepText>
      </StepContainerGroup>
      <StepContainerGroup>
        <StepNo>Finding 03</StepNo>
        <StepTitle>The worst experience is asked to go back to 10 days ago, not 1 day ago. </StepTitle>
        <StepText>"I don't mind if I redo 10 steps. But I can't redo all when somebody said to me he likes my design last week than the current one". Interviewees said they don't want every backup for a single action. Also, all of them think to create versions manually (not automatically) is better and easy to understand!</StepText>
      </StepContainerGroup>
    ...
    <Section>
      ...
      <SectionTitle>Deliver to early users before iteration. After each round of prototyping, I committed usability testing to know which part works well or not well. </SectionTitle>
      <InlineImage src={require("../images/sketch-clip-tests.png")} />
      <SmallTitle>Iteration</SmallTitle>
      <MainText>After the testings, I made some changes to my design based on the feedback. In the defect log I used to categorize, most of the feedbacks fell into confusing or error handling issues.</MainText>

      <StepContainerGroup>
        <StepNo>Iteration 01</StepNo>
        <StepTitle>Provide more clear instructions and an intro video at the beginning. </StepTitle>
        <StepText> “What if I close my Sketch but I forget to update the version” Obviously, users still have questions about this tool. That's why Sketch Clip needs more instant instructions for users. For example, when a user closes Sketch without committing, a pop-up will show up to notify the user.</StepText>
      </StepContainerGroup>
      <StepContainerGroup>
      ...
      <StepContainerGroup>
        ...
        <StepText>“I can’t understand why the versions are 4.0, 4.01, 4.01b. So what’s the next?” “Can I tell different branches by different colors- main branches or others?” To make the tree look simple, users now can fold the old version into one line. Also, in the new design, users can drag the branch to rearrange, for instance, to put the launched version in the middle line.</StepText>
      </StepContainerGroup>
    ...
    <NextNav currentPage={this.workId} list={props.location.state.list} />
